...
var Tracking = {
	marker : null,
	running : 0, // Indicate running state of the tracking 0 : desactvated, 1
	// configration ,2 running
	...
	start : function() {
		...
		if (Tracking.latlng == null) {
			if (Tracking.locationError != null) {
				...
				case 1:
					alert('Vous devez autoriser la localisation de votre appareil, sinon le tracking ne peut fonctionner.');
					break;
				default:
	...
	askMissionNumber : function(id_gare) {
		if (id_gare) {
			...
			$.get("http://www.raildar.fr/json/next_missions", args, function(data) {
				Tracking.loadingMessage.hide();
				console.log(data);
				...
				var html = $("<fieldset />").css({
					"max-height" : ($(window).height() * 8 / 10) + "px",
					"overflow" : "auto"
				}).render('tracking_gare_missions', {'missions' :data});
	...
	askNeighborMission : function(id_gare) {
		...
		$.get("http://www.raildar.fr/json/guess_my_train", args, function(data) {
			Tracking.loadingMessage.hide();
			console.log(data);
			...
			var html = $("<fieldset />").css({
				"max-height" : ($(window).height() * 8 / 10) + "px",
				"overflow" : "auto"
			}).render('tracking_near_missions', {'missions' :data});
